...
const create = catchError(async (req, res) => {
  ...
  // Synchronous option to generate token
  const code = require("crypto").randomBytes(64).toString("hex");

  // Setting values to the EmailCode model
  await EmailCode.create({
    code: code,
    userId: result.id,
  });

  // Sending the verification mail with nodemailer
  sendEmail({
    to: email,
...
const update = catchError(async (req, res) => {
  ...
  // Protect agaisnt attempts to change: email, password and isVerified status
...
const verifyUser = catchError(async (req, res) => {
  ...
  if (!userCode)
    return res.status(401).json({ error: "user not found to verify" });
...
const resetPassword = catchError(async (req, res) => {
  ...
  //Saving in the pivot table (database)
